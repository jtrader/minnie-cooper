...
export function Mt5PositionsCard() {
  ...
  const query = useQuery({
    ...
    retry: false,
    queryFn: () => request({ data: { resource: "positions" } }),
  });
  ...
  return (
    <section className="rounded-lg border border-border bg-card">
      ...
      <div className="overflow-x-auto p-3">
        {!connected ? (
          <p className="text-xs text-muted-foreground">Connect MetaTrader 5 to load positions.</p>
        ) : query.isLoading ? (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wide text-muted-foreground">
                ...
                <th className="pb-1 text-right">Swap</th>
            ...
            <tbody className="font-mono tabular-nums">
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border/60">
                  ...
                  <td className="py-1 text-right">{formatNumber(r.openPrice, 5)}</td>
                  <td className="py-1 text-right">{formatNumber(r.currentPrice, 5)}</td>
                  <td className={`py-1 text-right ${toneClass(r.swap)}`}>{formatNumber(r.swap, 2)}</td>
        ...
        {query.error ? (
          <p className="mt-2 text-xs text-destructive">
            {query.error instanceof Error ? query.error.message : "Could not load positions."}
          </p>
        ) : null}
      </div>
    </section>
